...
def load_nasa_battery_data(mat_file_path):
    """
    Loads a single NASA battery .mat file and extracts cycle data.
    This version uses explicit, robust indexing for NumPy structured arrays
    and does NOT use 'squeeze_me'.
    """
    try:
        # Load WITHOUT squeeze_me for a consistent, predictable structure.
        ...
        # Iterate through each cycle struct in the array
        for i in range(all_cycles.shape[0]):
            cycle = all_cycles[i]

            # --- DEFINITIVE FIX: CORRECT NUMPY STRUCTURED ARRAY ACCESS ---
            # Access the 'type' field's value, which is a nested array
            cycle_type = cycle['type'][0]
            if cycle_type != 'discharge':
                continue
            
            # Access the 'data' sub-struct
            cycle_data_struct = cycle['data'][0, 0]
            
            # Check for 'Capacity' within the sub-struct's fields (dtype.names)
            if 'Capacity' not in cycle_data_struct.dtype.names:
                continue
            
            capacity = cycle_data_struct['Capacity'][0][0]
            
            # Check for all required feature fields

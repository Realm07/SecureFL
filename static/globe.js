...
function createGlobe(container) {
    return new Promise((resolve, reject) => {
        ...
        function triggerPulse(clientId, retries = 5) {
            const key = `arc-${clientId}`;
            console.log(`[globe.js] triggerPulse: Attempting for Client #${clientId}. Arc key: '${key}'. Retries left: ${retries}`);
            if (retries <= 0) {
                console.error(`[globe.js] triggerPulse: FAILED for Client #${clientId}. Arc '${key}' not found after all retries.`);
                return;
            }
            if (activeArcs.has(key)) {
                console.log(`%c[globe.js] triggerPulse: SUCCESS for Client #${clientId}. Arc '${key}' found.`, 'color: #00FF00');
                createPulse(activeArcs.get(key).curve, 0x0077FF);
            } else {
                console.warn(`[globe.js] triggerPulse: Arc '${key}' not found. Retrying...`);
        ...
        function triggerBroadcastPulse(clientId, retries = 5) {
            ...
            if (activeArcs.has(key)) {
                console.log(`%c[globe.js] triggerBroadcastPulse: SUCCESS for Client #${clientId}. Arc '${key}' found.`, 'color: #00FF00');
                ...
                console.warn(`[globe.js] triggerBroadcastPulse: Arc '${key}' not found. Retrying...`);
